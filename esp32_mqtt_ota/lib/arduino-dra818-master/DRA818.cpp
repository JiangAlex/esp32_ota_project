...
int DRA818::read_response() {
  ...
  // 讀取完整回應直到超時
  while ((millis() - start) < TIMEOUT && buffer_index < 49) {
    if (this->serial->available()) {
      char c = this->serial->read();
      response_buffer[buffer_index++] = c;
      
      LOG(write, c);
      if (c >= 32 && c <= 126) {  // 只顯示可列印字元
        Serial.write(c);
      } else if (c == '\r') {
        Serial.print("<CR>");
      } else if (c == '\n') {
...
#ifdef DRA818_DEBUG
  if (!found_response) LOG(print, F("\r\n"));
#endif
  LOG(print, F("Returned value="));
  LOG(println, found_response);
...
int DRA818::handshake() {
  ...
  while (i-- > 0) {
    Serial.println("Sending: AT+DMOCONNECT");
    LOG(println, F("DRA818::handshake"));
    LOG(print, F("-> "));
    
    // 直接使用串列發送並確認
    Serial.print("Sending command directly... ");
    this->serial->print("AT+DMOCONNECT\r\n");
    this->serial->flush();  // 確保資料發送完成
    Serial.println("sent!");
